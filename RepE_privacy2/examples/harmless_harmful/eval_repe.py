...
def evaluate(prompts, layers, coef, model, tokenizer, rep_reader, template, component_index, num_new_tokens):
    ...
    activations = {}
    for layer in layers:
        activations[layer] = torch.tensor(coef * rep_reader.directions[layer][component_index] * rep_reader.direction_signs[layer][component_index]).to(model.device).half()
...
# Load prompts once (outside the loop)
...
for model_type in model_types:
    ...
    try:
        for label, layers in layer_id:
            ...
            results = evaluate(prompts, layers, coeff, model, tokenizer, rep_reader, template, component_index, num_new_tokens)
...
out_path = out_dir / "all_models_repE_100_test_cases.txt"
