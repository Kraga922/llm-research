    ...
    def _load_unembedding_matrix(self):
        ...
        try:
            if hasattr(self.model, 'lm_head'):
                ...
                self.unembedding_matrix = self.model.lm_head.weight.detach().to(torch.float32).cpu().numpy()  # [vocab_size, hidden_dim]
            elif hasattr(self.model, 'embed_out'):
                self.unembedding_matrix = self.model.embed_out.weight.detach().to(torch.float32).cpu().numpy()
            else:
                # Try to get from the embedding layer (transpose for unembedding)
                self.unembedding_matrix = self.model.get_input_embeddings().weight.detach().to(torch.float32).cpu().numpy()
            
            print(f"Loaded unembedding matrix: {self.unembedding_matrix.shape}")
        except Exception as e:
            print(f"Could not load unembedding matrix: {e}")
            # Try alternative approach for models without lm_head
            try:
                # For models that might not have lm_head, try to create it from embeddings
                embedding_weight = self.model.get_input_embeddings().weight.detach().to(torch.float32).cpu().numpy()
                # Use embedding matrix as unembedding (tied weights assumption)
                self.unembedding_matrix = embedding_weight
                print(f"Using embedding matrix as unembedding: {self.unembedding_matrix.shape}")
            except Exception as e2:
                print(f"Could not load embedding matrix either: {e2}")
                self.unembedding_matrix = None
            ...
            if directions:
                ...
                for layer_idx, direction in directions.items():
                    if hasattr(direction, 'cpu'):  # PyTorch tensor
                        direction_np = direction.to(torch.float32).cpu().numpy()
                    else:
...
def main():
    ...
    print("Initializing with gpt oos...")
    extractor = HarmfulSteeringTokenExtractor("openai/gpt-oss-20b")
    
    try:
        # Load the harmful directions (the steering vectors)
        extractor.load_harmful_directions("/home/ubuntu/krishiv-llm/neural_controllers/directions/rfm_harmful_gpt_oos.pkl")
        
        # Print comprehensive analysis
        extractor.print_steering_summary(top_k=15)
        
        # Create visualizations
        extractor.visualize_steering_tokens("harmful_steering_analysis_gpt_oos2.png")
        
        # Export detailed CSV
        df = extractor.export_steering_tokens_analysis("harmful_steering_tokens_gpt_oos2.csv")
