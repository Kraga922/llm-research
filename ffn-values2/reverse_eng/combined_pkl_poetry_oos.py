    ...
    def _load_unembedding_matrix(self):
        ...
        try:
            if hasattr(self.model, 'lm_head'):
                self.unembedding_matrix = self.model.lm_head.weight.detach().to(dtype=torch.float32).cpu().numpy()
            elif hasattr(self.model, 'embed_out'):
                self.unembedding_matrix = self.model.embed_out.weight.detach().to(dtype=torch.float32).cpu().numpy()
            else:
                self.unembedding_matrix = self.model.get_input_embeddings().weight.detach().to(dtype=torch.float32).cpu().numpy()
...
def main():
    ...
    extractor = SteeringTokenExtractor("openai/gpt-oss-20b")
    
    try:
        # Load steering directions
        extractor.load_steering_directions("/home/ubuntu/krishiv-llm/neural_controllers/directions/logistic_prose_gpt_oos.pkl")
        
        # Redirect stdout to capture printed analysis
        analysis_output = io.StringIO()
        sys_stdout_backup = sys.stdout  # Save original stdout
        sys.stdout = analysis_output    # Redirect prints to the StringIO buffer
        ...
        with open("steering_token_analysis.txt", "w") as f:
            f.write(analysis_output.getvalue())
        
        print("📝 Text-based analysis saved to 'steering_token_analysis_oss_p1.txt'")
        
        # Export detailed analysis as CSV
        df = extractor.export_steering_analysis("steering_tokens_analysis_oss_p1.csv", top_k=30)
        ...
        sys.stdout = sys_stdout_backup  # Restore stdout in case of error
        print(f"❌ Error: {e}")
